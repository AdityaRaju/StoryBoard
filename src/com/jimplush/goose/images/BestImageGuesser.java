...
public class BestImageGuesser implements ImageExtractor {
  ...
  public BestImageGuesser(Configuration config, HttpClient httpClient, String targetUrl,String domain) {
    ...
    boolean  checkMetaTagImage = false;
    public BestImageGuesser(Configuration config, HttpClient httpClient, boolean checkMetaTagImage,String domain) {
        this.httpClient = httpClient;
        ...
        this.checkMetaTagImage = checkMetaTagImage;
        this.imageDomain = domain;
  ...
  public Image getBestImage(Document doc, Element topNode) {
   ...
   if(image.getImageSrc() == null && topNode != null){
       checkForTopNodeImages(topNode);
   }
 ...
 private  boolean checkForTopNodeImages(Element topNode){
    try{
        //select all image nodes in topNode ....
         logger.info("Checking image by using TopNode logic...");
        //System.out.println("Checking image by using TopNode logic...");
        Elements imgNodes = topNode.select("img");
        if(imgNodes.size() > 0){
            Element firstImage = imgNodes.get(0);
            this.image.setImageSrc(firstImage.attr("src"));
            this.image.setImageExtractionType("s-custom");
  ...
  private boolean checkForMetaTag() {
    ...
    if (this.checkForLinkTag()) {
      ...
      //check with proper selector
    if (this.checkForOpenGraphTag("meta[name~=og:image]")) {
      return true;
    }
    //check using his style..
    if (this.checkForOpenGraphTag("meta[property~=og:image]")) {
          return true;
    }

      if (this.checkForOpenGraphTag("meta[property~=image]")) {
          return true;
      }
     ...
     * @return
     */
    private boolean checkForOpenGraphTag(String sel) {
        try {
            Elements meta = this.doc.select(sel);
  ...
  private void checkForLargeImages(Element node, int parentDepth, int siblingDepth) {
    ...
    if (highScoreImage != null) {
      ...
      this.image.setImageExtractionType("bigimage");
        //todo : commented for no need of bytes..
  ...
  private String buildImagePath(String image) {
    ...
    try {
      ...
      URL imageURL = new URL("http",this.imageDomain, image);
   ...
   *  //HARI- Commented by me...............un comment if we need to get the image bytes...
   * @param src
   * @return
   */
...
*/
  ...
  private HashMap<Element, Float> downloadImagesAndGetResults(ArrayList<Element> images, int depthLevel) {
    ...
    for (Element image : images) {
      //TODO: Commented for not downloading....  IMP..........need to take this, when need to find the image width and height
      if (cnt > 0) {
        if (logger.isDebugEnabled()) {
